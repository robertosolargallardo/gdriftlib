...
//		cout<<"ModelWF::run - Iterando por "<<dst->size()<<" individuos ("<<profile->getChromosomes()<<" chromosomes, "<<profile->getGenes(0)<<" genes in ch0)\n";
		NanoTimer timer;
...
//		Individual *ind = src->at(0);
		// Padre elegido para el individuo de dst
...
//			parent = src->at(src_dist(rng));
//			dst->at(id)->setParent(parent);
			parent = src_dist(rng);
...
//				cout<<"ModelWF::run - total_muts: "<<total_muts<<" ("<<(length * dst->size())<<", "<<rate<<")\n";
				
				for(unsigned int mut = 0; mut < total_muts; ++mut){
					// Escoger individuo para mutar
					unsigned int mut_pos = dst_dist(rng);
//					cout<<"ModelWF::run - mut_pos: "<<mut_pos<<" de "<<dst->size()<<"\n";
					
					// Creo una nueva secuencia con el gen actual del individuo para mutar y reemplazar
					VirtualSequence *original = dst->at(mut_pos).getGene(genid, chrid, 0);
//					cout<<"ModelWF::run - Crando secuencia para mutar (original NULL? "<<(original==NULL)<<", size: "<<((original!=NULL)?original->length():0)<<")\n";
					seq = new VirtualSequence(*original);
//					cout<<"ModelWF::run - mutate...\n";
					seq->mutate();
					++mutations;
//					cout<<"ModelWF::run - push...\n";
					pool->push(chrid, genid, seq);
//					cout<<"ModelWF::run - setGene...\n";
					dst->at(mut_pos).setGene(genid, chrid, 0, seq);
//					cout<<"ModelWF::run - Fin\n";
				}
			}
		}
